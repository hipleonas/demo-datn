...
"""
Step 1: python3 ./zipvoice/bin/load_vimd.py --output-dir data/vimd/manifests --audio-dir data/vimd/audio --num-jobs 4 --max-items 100
đầu tiên ta cần tạo ra manifest cho tập huấn luyện

python3 ./zipvoice/bin/prepare_dataset.py \
    --tsv-path /path/to/your/train.tsv \
    --prefix "my_vietnamese_data" \
    --subset "train" \
    --output-dir "data/manifests" \
    --num-jobs 10

# Tạo manifest cho tập kiểm định
python3 ./zipvoice/bin/prepare_dataset.py \
    --tsv-path /path/to/your/dev.tsv \
    --prefix "my_vietnamese_data" \
    --subset "dev" \
    --output-dir "data/manifests" \
    --num-jobs 10

Sau đó đi tính toán compute_fbank.py để tính toán fbaj features ta 
ta tính toán từ các manifests vừa tạo ra

 python3 -m zipvoice.bin.compute_fbank \
    --source-dir data/vimd/manifests \
    --dest-dir data/fbank \
    --dataset "vimd" \
    --subset "train" \
    --num-jobs 16
...
# Trích xuất FBank cho tập kiểm định
 python3 -m zipvoice.bin.compute_fbank \
    --source-dir data/vimd/manifests \
    --dest-dir data/fbank \
    --dataset "vimd" \
    --subset "validation" \
    --num-jobs 16
--source-dir: Thư mục chứa manifest từ Bước 1.

--dest-dir: Thư mục để lưu manifest mới đã có FBank. Sau bước này, bạn sẽ có các file tương ứng trong data/fbank.

Bước 3: huấn luyện và tinh chỉnh thông số mooh hình
python3 ./zipvoice/bin/train_zipvoice.py \
    --world-size 1 \
    --num-epochs 10 \
    --max-duration 400 \
    --exp-dir exp/zipvoice_finetune_vimd \
    --model-config exp/zipvoice_finetune_vimd/config.json \
    --token-file exp/zipvoice_finetune_vimd/tokens.txt \
    --dataset custom \
    --train-manifest data/fbank/my_vietnamese_data_cuts_train.jsonl.gz \
    --dev-manifest data/fbank/my_vietnamese_data_cuts_dev.jsonl.gz \
    --checkpoint /path/to/your/pretrained/epoch-11.pt \
    --finetune True \
    --base-lr 1e-5
...
python3 -m zipvoice.bin.train_zipvoice \
    ...
    --token-file exp/zipvoice_finetune_vimd/tokens.txt \
    --dataset custom \
    --train-manifest data/fbank/vimd_cuts_train.jsonl.gz \
    --dev-manifest data/fbank/vimd_cuts_validation.jsonl.gz \
    --checkpoint /path/to/your/pretrained/epoch-11.pt \
    --finetune True \
    --base-lr 1e-5 \
    --save-every-n 2000 \
    ...
    (.venv) hiepquoc@LAPTOP-HVDMQOFI:~/ZipVoice$ python3 -m zipvoice.bin.train_zipvoice \
    --world-size 1 \
    --num-epochs 2 \
    --max-duration 400 \
    --exp-dir exp/zipvoice_finetune_vimd \
    --model-config zip_2500/model.json \
    --token-file zip_2500/tokens.txt \
    --dataset custom \
    --train-manifest data/fbank/vimd_cuts_train.jsonl.gz \
    --dev-manifest data/fbank/vimd_cuts_validation.jsonl.gz \
    --checkpoint zip_2500/iter-525000-avg-2.pt \
    --finetune True \
    --base-lr 1e-5 \
    --save-every-n 2000 \
    --valid-by-epoch False 
