...
def get_parser():
    ...
    parser.add_argument(
        ...
        after every save_every_n iterations. Ignored if --dev-manifest is not provided.
        """,
    )
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=True,
        help="""Whether to run validation at the end of training. Only used when 
        --dev-manifest is provided.
        """,
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path of the validation manifest (optional - if not provided, validation will be skipped during training)",
    )
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(train_dl):
        ...
        # Only run validation if valid_dl is provided
        if valid_dl is not None and (
        ...
        try:
            ...
            # save_bad_model() #Tạm thời tắt save_bad_model
            raise
...
def run(rank, world_size, args):
    ...
    if params.dataset == "emilia":
        ...
        train_cuts = train_cuts.filter(_remove_short_and_long_utt)
        dev_cuts = datamodule.dev_libritts_cuts() if params.dev_manifest else None
    else:
        assert params.dataset == "custom"
        train_cuts = datamodule.train_custom_cuts(params.train_manifest)
        train_cuts = train_cuts.filter(_remove_short_and_long_utt)
        dev_cuts = datamodule.dev_custom_cuts(params.dev_manifest) if params.dev_manifest else None
        # To avoid OOM issues due to too long dev cuts
        if dev_cuts is not None:
            dev_cuts = dev_cuts.filter(_remove_short_and_long_utt)

    if params.tokenizer in ["emilia", "espeak", "dialog"]:
        train_needs_tokenization = not hasattr(train_cuts[0].supervisions[0], "tokens")
        dev_needs_tokenization = dev_cuts is not None and not hasattr(
            dev_cuts[0].supervisions[0], "tokens"
        )
        if train_needs_tokenization or dev_needs_tokenization:
            logging.warning(
    ...
    valid_dl = datamodule.dev_dataloaders(dev_cuts) if dev_cuts is not None else None
    ...
    if valid_dl is None:
        logging.info("Training started (no validation dataset provided - validation will be skipped during training)")
    else:
        logging.info("Training started (validation dataset provided - will validate during training and at the end)")
    ...
    # Run final validation if validation dataset is provided and validate_at_end is True
    if valid_dl is not None and params.validate_at_end:
        logging.info("Running final validation after training completion")
        final_valid_info = compute_validation_loss(
            params=params,
            model=model,
            valid_dl=valid_dl,
            world_size=world_size,
        )
        logging.info(f"Final validation results: {final_valid_info}")
        
        if tb_writer is not None:
            final_valid_info.write_summary(
                tb_writer, "train/final_valid_", params.batch_idx_train
            )
            
        # Save final validation checkpoint if it's the best
        if rank == 0 and params.best_valid_epoch == params.cur_epoch:
            final_best_filename = params.exp_dir / "final-best-valid-loss.pt"
            epoch_filename = params.exp_dir / f"epoch-{params.cur_epoch}.pt"
            if epoch_filename.exists():
                copyfile(src=epoch_filename, dst=final_best_filename)
                logging.info(f"Saved final best validation model to {final_best_filename}")
